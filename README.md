...
| 단계 | CLI 명령 | 설명 |
|------|----------|------|
| 0a | `pptx2pdf` | `.pptx` → born-digital PDF 변환 (`data/raw/` → `data/pdf/`), python-pptx + LibreOffice headless (크로스플랫폼) |
...
# 0a. PPTX → born-digital PDF 변환 (python-pptx + LibreOffice headless, 크로스플랫폼)
python -m docpipe pptx2pdf --raw_dir data/raw --pdf_dir data/pdf
# 이미 존재하는 PDF 덮어쓰기: --overwrite 옵션 추가
# LibreOffice 설치 필요: https://www.libreoffice.org/  또는 winget install TheDocumentFoundation.LibreOffice
...
- python-pptx로 PPTX 유효성 검증 후 LibreOffice headless CLI(`soffice`)로 PDF 변환
- LibreOffice가 PATH에 없을 경우 표준 설치 경로 자동 탐색
- LibreOffice 설치: `winget install TheDocumentFoundation.LibreOffice` (Windows) 또는 `apt install libreoffice` (Linux)
